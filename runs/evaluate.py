...
class GradCAM:
    """
    针对 EfficientNetB3WithCBAM 的 Grad-CAM 实现
...
def visualize_gradcam(model_baseline, model_improved,
    ...
    # 取第一个 batch 中前 num_samples 张正确分类样本
    imgs_all, labels_all = [], []
    ...
    # Grad-CAM hooks
    cam_base = GradCAM(model_baseline,
                        model_baseline.backbone.conv_head)
    cam_imp  = GradCAM(model_improved,
                        model_improved.backbone.conv_head)
...
def main():
    ...
    # ── 改进模型 ──
    model = EfficientNetB3WithCBAM(num_classes=args.num_classes).to(device)
    ckpt  = torch.load(args.ckpt, map_location=device)
    model.load_state_dict(ckpt["model"])
    print(f"加载改进模型: {args.ckpt}  (val_acc={ckpt.get('val_acc', 'N/A')})")

    # ── 推理 ──
    preds, labels, probs = predict(model, loaders["test"], device)

    # ── 指标 ──
    metrics = compute_metrics(preds, labels, class_names, save_dir)

    # ── 混淆矩阵 ──
    plot_confusion_matrix(preds, labels, class_names, save_dir)

    # ── 每类准确率 ──
    plot_per_class_accuracy(preds, labels, class_names, save_dir)

    # ── Grad-CAM（需要提供基线权重）──
    if args.ckpt_baseline:
        baseline = EfficientNetB3WithCBAM(
    ...
    (save_dir / "metrics.txt").write_text(result_txt, encoding="utf-8")
    print("\n所有评估结果保存至:", save_dir)
